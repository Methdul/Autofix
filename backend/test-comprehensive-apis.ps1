...
# Tests all 18 endpoints across 5 modules (Auth, Vehicle, Provider, Booking, Invoice)
...
Write-Host "      Vehicle Service Ecosystem - Comprehensive API Test Suite" -ForegroundColor Cyan
Write-Host "================================================================`n" -ForegroundColor Cyan
...
#═══════════════════════════════════════════════════════════
# STEP 0: Health Check (1 endpoint)
#═══════════════════════════════════════════════════════════
...
#═══════════════════════════════════════════════════════════
# STEP 1: Authentication Module (2 endpoints - Register, Login)
#═══════════════════════════════════════════════════════════
...
Test-Endpoint "POST /api/auth/register - Register Owner" {
    ...
    # We won't save token here, we will test login to do that
    $script:ownerId = $response.user.id
    Write-Host "  Owner Registered (ID: $($script:ownerId))" -ForegroundColor Gray
}
...
Test-Endpoint "POST /api/auth/register - Register Provider" {
    ...
    Write-Host "  Provider Registered (ID: $($script:providerId))" -ForegroundColor Gray
}

Test-Endpoint "POST /api/auth/login - Login Owner" {
    $body = @{
        email    = $ownerEmail
        password = $password
    } | ConvertTo-Json
    ...
    Write-Host "  Owner Logged In Successfully" -ForegroundColor Gray
}

Test-Endpoint "POST /api/auth/login - Login Provider" {
    $body = @{
        email    = $providerEmail
        password = $password
    } | ConvertTo-Json
    ...
    Write-Host "  Provider Logged In Successfully" -ForegroundColor Gray
}
...
Test-Endpoint "GET /api/providers/:id - Get Provider Details (Public)" {
    ...
    # Notice this does not require authentication headers based on the routes
...
Test-Endpoint "PATCH /api/bookings/:id/status - Update to COMPLETED" {
    ...
    # We will simulate the state machine: PENDING -> ACCEPTED -> IN_PROGRESS -> COMPLETED
    $body1 = @{ status = "ACCEPTED" } | ConvertTo-Json
    Invoke-RestMethod -Uri "$baseUrl/api/bookings/$($script:bookingId)/status" -Method PATCH -Body $body1 -Headers $providerHeaders -ContentType "application/json" | Out-Null
    
    $body2 = @{ status = "IN_PROGRESS" } | ConvertTo-Json
    Invoke-RestMethod -Uri "$baseUrl/api/bookings/$($script:bookingId)/status" -Method PATCH -Body $body2 -Headers $providerHeaders -ContentType "application/json" | Out-Null
    
    $body3 = @{ status = "COMPLETED" } | ConvertTo-Json
    $response = Invoke-RestMethod -Uri "$baseUrl/api/bookings/$($script:bookingId)/status" -Method PATCH -Body $body3 -Headers $providerHeaders -ContentType "application/json"
...
Test-Endpoint "GET /api/invoices - Get Owner's Invoices" {
    ...
    Write-Host "  Found $($response.Count) invoice(s)" -ForegroundColor Gray
} # Note this was testing 2 endpoints (owner and provider GET /invoices) 

#═══════════════════════════════════════════════════════════
# STEP 6: Cleanup Module endpoints
#═══════════════════════════════════════════════════════════
Write-Host "`n--- STEP 6: Cleanup ---" -ForegroundColor Magenta
